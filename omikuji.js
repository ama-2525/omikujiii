...
assessmentButton.onclick = () => {
    ...
    //入力フォームを削除し、もう一度引くボタン・シェアボタンを表示
    const clear = document.getElementById('top-area');
    clear.style.display = "none";
    const on = document.getElementById('button*2');
    on.classList.remove('off');

    const oneMore = document.getElementById('one-more');
    const anchorO = document.createElement('a');
    anchorO.textContent = "もう一度引く"
    const hrefValueO = '/Users/amaaa/Desktop/school/omikujiii/omikuji.html'
    anchorO.setAttribute('href', hrefValueO);
    oneMore.appendChild(anchorO);

    const tweet = document.getElementById('tweet');
    const anchorT = document.createElement('a');
    anchorT.textContent = "ツイートでシェア"
    const hrefValueT =' https://twitter.com/intent/tweet?button_hashtag=' + encodeURIComponent('おみくじ結果') + '&ref_src=twsrc%5Etfw';
    anchorT.setAttribute('href', hrefValueT);
    tweet.appendChild(anchorT);
